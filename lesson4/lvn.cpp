...
json make_lvn_val(
    json instr, std::vector<std::pair<json, std::string>> const &idx_value_name,
...
json lvn(const json &program) {
  ...
  for (auto const &func : program["functions"]) {
    ...
    for (BasicBlock &block : basic_blocks) {
      ...
      std::vector<std::pair<json, std::string>>
          idx_value_name; // idx: 0, ({const 4}, x)
        ...
        if (instr.contains("dest")) {
          ...
          if (instr.contains("args")) {
            for (auto arg : instr["args"]) {
              // if the arg has not been defined, create a mapping from it to
              // itself (i.e. x -> id x)
      ...
      for (auto instr : block.instructions) {
        ...
        if (op == "print") {
          ...
          instr["args"] =
              json::array({idx_value_name[variable_index[print_arg]].second});
          curr_function["instrs"].push_back(instr);
          continue;
        } else if (op == "ret" or op == "jmp" or op == "call" or
                   op == "label" or op == "br") {
          curr_function["instrs"].push_back(instr);
          continue;
        }
        ...
        if (value_number == -1) {
          ...
          if (var_def_count[dest] > 1) {
            var_def_count[dest]--;
            std::string mangled =
                std::string(var_def_count[dest], '_') + dest; // 5*'_' + dest
